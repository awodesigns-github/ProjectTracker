@extends('layouts.app')
@section('content')
<div class="block-header">
    <div class="row align-items-center">
        <div class="col-lg-6 col-md-6 col-sm-12">
            <h2>Project Information</h2>
            <ul class="breadcrumb">
                <li class="breadcrumb-item"><a href="#"><i class="fa fa-dashboard"></i></a></li> 
                <li class="breadcrumb-item">My Projects</li>
                <li class="breadcrumb-item">{{ $project->name }}</li>                           
            </ul>
        </div>
    </div>
</div>

<div class="card" style="border: 1px solid rgb(203, 202, 202)">
        <div class="mail-inbox">
            <div class="mail-left collapse" id="email-nav">
                <div class="mail-compose m-b-20">
                    <a href="#" class="btn btn-block btn-primary"><span class="pr-1 fa fa-check"></span> Mark Project as Done</a>
                    <hr class="my-2">
                </div>
                <div class="mail-side" id="preCheck">
                    <ul  class="nav nav-tabs-new list-unstyled">
                        <li class="nav-item"><a class="nav-link show active" data-toggle="tab" href="#details"><b>General Information</b></a></li>
                        <li class="nav-item"><a class="nav-link" data-toggle="tab" href="#tasks"><b>Tasks</b></a></li>
                    </ul>
                </div>
            </div>
            
            <div class="mail-right">
                    <div class="body">
                        <div class="tab-content">
                            <div class="tab-pane active" id="details">
                                <h4>Details</h4>
                                <hr>
                                <div class="pb-4"></div>
                                <div class="row">
                                    <div class="col-lg-6">
                                        <div class="pb-2 d-flex">
                                            <label for="Name"><b>Name : </b></label>
                                            <p class="pl-2">{{ $project->name }}</p>
                                        </div>
                                        <div class="pb-2 d-flex">
                                            <label for="Name"><b>Description : </b></label>
                                            <p class="pl-2">{{ $project->description }}</p>
                                        </div>
                                        <div class="pb-2 d-flex">
                                            <label for="Name"><b>Github : </b></label>
                                            <p class="pl-2"> <a href="https://www.github.com/{{ $studentInformation->github_username }}" target="_blank">www.github.com/{{ $studentInformation->github_username }}/{{ $project->name }} <i class="fa fa-level-up"></i></a></p>
                                        </div>
                                        <div class="pb-2 d-flex">
                                            <label for="Name"><b>Due date : </b></label>
                                            <p class="pl-2">{{ $project->due_date }}</p>
                                        </div>
                                    </div>
                                    <div class="col-lg-6">
                                        <div class="pb-2 d-flex">
                                            <label for="Name"><b>Module : </b></label>
                                            <p class="pl-2">{{ $project->module->name }}</p>
                                        </div>
                                        <div class="pb-2 d-flex">
                                            <label for="Name"><b>Status : </b></label>
                                            <p class="pl-2">{{ $project->status == 'O' ? 'Open' : 'Closed' }}</p>
                                        </div>
                                        <div class="pb-2 d-flex">
                                            <label for="Name"><b>Created at : </b></label>
                                            <p class="pl-2">{{ $project->created_at }}</p>
                                        </div>
                                        <div class="pb-2 d-flex">
                                            <label for="Name"><b>Time remaining (in days) : </b></label>
                                            <p class="pl-2">{{ $project->days_remaining }}</p>
                                        </div>
                                    </div>
                                </div>
                                <hr>
                            </div>
                            
                            <div class="tab-pane" id="tasks">
                                <h4>My Tasks</h4>
                                <hr>
                                <div class="row">
                                    <div class="body table-responsive">
                                        <table class="table table-bordered table-striped table-hover dataTable js-exportable table table-hover mb-0 c_list">
                                            <thead>
                                                <tr>
                                                    <th>Tasks</th>
                                                    <th>Status</th>
                                                    <th>Date of Submission</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach ($tasks as $task)
                                                <tr>
                                                    <td class="scope">{{ $task->task_name }}</td>
                                                    <td>{{ $task->pivot->status == 'C' ? 'Completed' : 'Pending' }}</td>
                                                    <td>N/A</td>
                                                </tr>
                                                @endforeach
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                                <hr>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection